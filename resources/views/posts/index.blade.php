...
@section('content')
    ...
    <div class="col-md-10 col-md-offset-1">
        ...
        <div class="panel panel-default">
            ...
            <form method="POST" role="form" enctype="multipart/form-data" action="{{ route('Posts.search') }}">
             {{ csrf_field() }}
             @component('layouts.search', ['title' => 'Search'])
              @component('layouts.two-cols-search-row', ['items' => ['Title', 'Content'], 
              'oldVals' => [isset($searchingVals) ? $searchingVals['title'] : '', isset($searchingVals) ? $searchingVals['content'] : '']])
              @endcomponent
            @endcomponent
          </form>
                        ...
                        <tbody>
                            @foreach($posts as $indexKey => $post)
                                <tr class="item{{$post->id}} @if($post->is_published) warning @endif">
                                    ...
                                    <td>{{$post->name}} 
                                    <br>
                                    <img src="../{{$post->picture }}" width="100px" height="100px"/></td>
